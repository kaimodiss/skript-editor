...
let errorRegs = [{
		reg: /\s+(command|function) .+:/g,
		msg: "Declerations of commands & functions should not have whitespaces behind them"
	},
	{
		reg: /^(trigger|description|cooldown|permission)([/\w]+|):([/\w+]|)/g,
		msg: "Command properties should have whitespaces behind them"
	},
	{
		reg: /teleport (the |)(player|attacker|victim|loop-entity|loop-player|) (to|below|above|next to) (-|)\d+(,|) (-|)\d+(,|) (-|)\d+/g,
		msg: "Use vector(x, y, z) instead of x, y, z"
	},
	{
		reg: /^{_\w+}/g,
		...
		if (editor.getValue() == "") {
			...
			editor.getValue().split("\n").forEach((txt, index) => {
				errorRegs.forEach(data => {
					if (txt.match(data.reg)) annData.push({
						row: index,
						column: 0,
						text: data.msg,
...
$(() => {
	swal({
		...
		tempSocket.on("verified", text => {
			...
			tempSocket.on("userDisconnect", e => new Toast({
				message: `"${e}" disconnected!`
			}))
		})
	...
	try {
		setTimeout(() => Cookies.get("blastCode") ? editor.setOption('blastCode', {
			effect: 1
		}) : editor._codeBlast.destroy(), 200)
	} catch (e) {
...
$("#blast-o").change(() => {
	$("#blast-o").is(':checked') ? editor.setOption('blastCode', {
		effect: 1
